...
class Attention(torch.nn.Module):
    ...
    def train(self, mode=True):
        ...
        if mode and hasattr(self, "ab"):
            ...
            self.ab = self.attention_biases[:, self.attention_bias_idxs].cuda()

    def forward(self, x, graph_context=None):  # x (B,N,C)
        B, N, _ = x.shape
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x, graph_context=None):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        res_x = x
        if H == self.window_size and W == self.window_size:
            x = self.attn(x, graph_context=graph_context)
        else:
            x = x.view(B, H, W, C)
    ...
    def forward(self, x, graph_context=None):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x, graph_context=graph_context)
        if self.downsample is not None:
            x = self.downsample(x)
        return x
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x, graph_context=None):
        # x: (N, C, H, W)
        ...
        # Get diff along dimension 1 (temporal dimension)
        if graph_context is not None:
            graph_context_diff = graph_context.diff(dim=1).diff(dim=1)
            B, T_diff, current_nodes, feat = graph_context_diff.shape
            # Interpolate dimension 2 from 47 to 494
            target_nodes = 49
            # Reshape for interpolation: (B*T_diff*feat, current_nodes)
            reshaped = graph_context_diff.permute(0, 1, 3, 2).reshape(-1, current_nodes)
            # Interpolate to target size
            interpolated = F.interpolate(
                reshaped.unsqueeze(0), size=49, mode="linear", align_corners=False
            )
            # Reshape back to original format: (B, T_diff, target_nodes, feat)
            graph_context_resized = (
                interpolated.squeeze(0)
                .reshape(B, T_diff, feat, target_nodes)
                .permute(0, 1, 3, 2)
            )
            graph_context = graph_context_resized[:, 0, :, :]

        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x = layer(x, graph_context=graph_context)
    ...
    def forward(self, x, graph_context=None):
        x, x_feat = self.forward_features(x, graph_context=graph_context)
        x = self.norm_head(x)
        x = self.head(x)
        return x, x_feat
